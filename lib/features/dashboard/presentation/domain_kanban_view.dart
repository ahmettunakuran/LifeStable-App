    ...
    return BlocBuilder<TasksBloc, TasksState>(
      builder: (context, state) {
        if (state is TasksLoading) {
          ...
          return Center(child: Text(state.message, style: TextStyle(color: Colors.white.withOpacity(0.5))));
        }
        return Center(child: Text('No tasks found for this domain.', style: TextStyle(color: Colors.white.withOpacity(0.4))));
      },
    );
  }
        ...
        children: [
          Padding(
            ...
            child: Row(
              ...
              children: [
                Text(title, style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12, letterSpacing: 1.2, color: Colors.white.withOpacity(0.5))),
                Container(
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: Column(
          ...
          children: [
            Container(height: 3, decoration: BoxDecoration(gradient: LinearGradient(colors: [AppColors.goldLight.withOpacity(0.6), AppColors.gold.withOpacity(0.3)]))),
            Padding(
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      if (task.dueDate != null)
                        Row(children: [
                          Icon(Icons.calendar_today, size: 11, color: Colors.white.withOpacity(0.3)),
                          const SizedBox(width: 4),
                          Text(DateFormat('MMM d').format(task.dueDate!),
                              style: TextStyle(fontSize: 11, color: Colors.white.withOpacity(0.3), fontWeight: FontWeight.w600)),
                        ]),
                    ],
                  ),
                ],
...
class _TaskActions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<dynamic>(
      ...
      onSelected: (value) {
        if (value is TaskStatus) onStatusChanged(value);
        else if (value == 'delete') onDelete?.call();
      },
      icon: Icon(Icons.more_horiz, size: 18, color: Colors.white.withOpacity(0.3)),
      color: AppColors.cardBg,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
      itemBuilder: (context) => [
        PopupMenuItem(enabled: false, child: Text('MOVE TO', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, color: Colors.white.withOpacity(0.3)))),
