        ...
        return BlocBuilder<LocationCubit, LocationState>(
          builder: (context, state) {
            ...
            if (state.status == LocationStatus.error) {
              return Center(
                child: Text(
                  state.errorMessage ?? S.of('generic_error'),
  ...
  Future<void> _confirmDelete(BuildContext context, LocationEntity location) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text(S.of('delete_location_confirm_title'), style: const TextStyle(color: Colors.white)),
        content: Text(
          S.of('delete_location_confirm_body').replaceAll('{label}', location.label),
