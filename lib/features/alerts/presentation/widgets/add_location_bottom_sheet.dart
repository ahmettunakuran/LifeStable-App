...
class _AddLocationBottomSheetState extends State<AddLocationBottomSheet> {
  ...
  Future<void> _searchAddress() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _searchError = S.of('search_error_not_found');
          _isSearching = false;
        });
      }
    }
  ...
  Future<void> _save() async {
    if (_selectedPosition == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(S.of('pick_location_warning'))),
      );
      return;
    }
    if (_labelController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(S.of('enter_label_warning'))),
      );
      return;
    }
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    Text(S.of('radius_label'), style: const TextStyle(color: Colors.white70, fontSize: 14)),
                    Text(
                      '${_radius.round()} m',
